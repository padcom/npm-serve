...
import { getETagFor, runSingleWithLockAndKey } from './utils.mjs'
...
export default function({
  ...
  router.param('coordinates', async (request, response, next, coordinates) => {
    ...
    if (await cache.isCached(pkg.fullname)) {
      ...
      if (pkg.metadata.isOutdated()) {
        pkg.metadata = await runSingleWithLockAndKey(pkg.lock, 'metadata', () => refreshPackageMetadata(pkg))
      }
  ...
  router.get('/:coordinates([^/]*)', async (request, response) => {
    ...
    try {
      if (pkg.version !== pkg.selectedVersion || pkg.path !== pkg.selectedPath) {
        ...
        if (!exists(archive)) {
          await runSingleWithLockAndKey(pkg.lock, 'tarball', () => npm.downloadPackageTarball(pkg.scope, pkg.name, pkg.version, archive))
        }
  ...
  async function refreshPackageMetadata(pkg) {
