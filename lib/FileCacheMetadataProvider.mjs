...
export class FileCacheMetadataProvider {
  ...
  async extract(fullname, version, path, output) {
    const archive = this.getArchiveFileName(fullname, version)
    try {
      await extract(archive, 'package/' + path, output)
    } catch (e) {
      if (e !== 'Not found (in archive)') {
        logger.error(e)
        logger.error('Error while extracting ' + path + ' from ' + this.getArchiveFileName(fullname, version))
        logger.error('It is possible that the local cache has been corrupted. It will be removed. If the problem persists create a Github issue - it will be taken care of!')
        await unlink(archive)
      } else {
        logger.warn(`Path ${path} not found in ${archive}`)
      }
    }
  }
}
